...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="bg-white shadow-2xl rounded-xl overflow-hidden border border-gray-100">
        ...
        <div class="bg-indigo-600 px-8 py-6 text-white flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold tracking-tight">Edit Departemen</h1>
                <p class="text-indigo-100 text-sm mt-1">
                    Perbarui informasi untuk departemen: <span class="font-semibold text-white underline decoration-indigo-400 underline-offset-2">{{ $department->name }}</span>
                </p>
            </div>
            <div class="opacity-20 hidden md:block">
                {{-- Icon Edit/Pencil pada Dokumen --}}
        ...
        <form action="{{ route('departments.update', $department->uuid) }}" method="POST" class="p-8 md:p-10">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="relative">
                        <textarea name="description" id="description" rows="4" 
                            class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 border-gray-300 bg-gray-50 focus:bg-white">{{ old('description', $department->description) }}</textarea>
                    </div>
                </div>
