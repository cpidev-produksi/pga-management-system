...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl md:text-3xl text-gray-800 font-bold">Tong Sampah User 🗑️</h1>
            <p class="text-sm text-gray-500 mt-1">Mengelola user yang telah dihapus sementara.</p>
        </div>
        
        <a href="{{ route('users.index') }}" class="inline-flex justify-center items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th scope="col" class="px-6 py-4 font-bold tracking-wider">Nama User</th>
                        <th scope="col" class="px-6 py-4 font-bold tracking-wider text-center">Role</th>
                        <th scope="col" class="px-6 py-4 font-bold tracking-wider">Departemen</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="bg-white hover:bg-red-50 transition-colors duration-200">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <div class="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold border-2 border-white shadow-sm">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($user->department)
                                <div class="text-gray-700 font-medium">{{ $user->department->name }}</div>
                            @else
                                <span class="text-gray-400 text-xs italic">- No Dept -</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <form action="{{ route('users.restore', $user->uuid) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="group p-2 rounded-lg border border-transparent hover:bg-green-50 hover:border-green-200 transition-all duration-200 flex items-center gap-1 text-green-600 font-medium text-xs uppercase tracking-wider" title="Kembalikan User">
                                ...
                                <form action="{{ route('users.force-delete', $user->uuid) }}" method="POST" onsubmit="return confirm('PERINGATAN: User akan dihapus SELAMANYA dan tidak bisa dikembalikan. Lanjutkan?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm mt-1">Tidak ada user yang dihapus sementara.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
