    ...
    <div class="bg-white shadow-2xl rounded-xl overflow-hidden border border-gray-100">
        ...
        <div class="bg-indigo-600 px-8 py-6 text-white flex justify-between items-center">
            <div>
                ...
                <p class="text-indigo-100 text-sm mt-1">Perbarui informasi pengguna: <strong>{{ $user->name }}</strong></p>
            </div>
            <div class="opacity-20 hidden md:block">
                <svg class="w-16 h-16" fill="currentColor" viewBox="0 0 20 20"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
            </div>
        </div>

        <form action="{{ route('users.update', $user->uuid) }}" method="POST" class="p-8 md:p-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <div class="relative">
                            <select name="role_uuid" id="role_uuid" required
                                ...
                                @php($currentRoleUuid = old('role_uuid', optional($user->roles->first())->uuid))
                                @foreach($roles as $role)
                                    <option value="{{ $role->uuid }}" data-super="{{ $role->name === 'Super Admin' ? '1' : '0' }}"
                                        {{ $currentRoleUuid == $role->uuid ? 'selected' : '' }}>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="department_uuid" id="department_uuid"
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->uuid }}" 
                                        {{ old('department_uuid', $user->department_uuid) == $dept->uuid ? 'selected' : '' }}>
                                        {{ $dept->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="plant_uuid" id="plant_uuid"
                                ...
                                @foreach($plants as $plant)
                                    <option value="{{ $plant->uuid }}"
                                        {{ old('plant_uuid', $user->plant_uuid) == $plant->uuid ? 'selected' : '' }}>
                ...
                <div class="col-span-1 md:col-span-2">
                    <div class="flex items-start p-4 border border-gray-200 rounded-lg bg-gray-50 hover:bg-white hover:border-indigo-300 transition-colors">
                        <div class="flex items-center h-5">
                            <input id="is_contactable" name="is_contactable" type="checkbox" value="1" 
                                {{ old('is_contactable', $user->is_contactable) ? 'checked' : '' }}
                ...
                <div class="col-span-1 md:col-span-2 py-2">
                    ...
                    <div class="mt-4 flex items-center p-4 mb-2 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50" role="alert">
                        <svg class="flex-shrink-0 inline w-4 h-4 mr-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
                        </svg>
                        <span class="sr-only">Info</span>
                        <div>
                            <span class="font-medium">Info:</span> Kosongkan kolom password di bawah jika Anda tidak ingin mengubah password saat ini.
                        </div>
                    </div>
                    <p class="text-xs text-gray-500 mt-2 uppercase tracking-wider font-bold">Ganti Password (Opsional)</p>
                </div>
